<template>
    <button @click="reppleClick" :class="{active: repple_button.toggle}" ref="rippleButton">
        <slot></slot>
    ...
    data () {
     ...
     methods: {
        reppleClick (e) {
           ...
           // let button = e.target
            let button = this.$refs.rippleButton
...
<style>
        .__cov-button-ripple {
            ...
            position: relative;
            height: 36px;
            min-width: 64px;    padding: 0 16px;
            display: inline-block;    font-family: Roboto,Helvetica,Arial,sans-serif;
